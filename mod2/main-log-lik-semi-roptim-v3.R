...
if(opt$kcontext == 4){
  ...
  for(i in 1:1500){
    ...
    aux_tpm_row1 = rdirichlet(4,alpha=c(10,1,1))
    aux_tpm_row2 = rdirichlet(4,alpha=c(1,10,1))
    aux_tpm_row3 = rdirichlet(4,alpha=c(1,1,10))
    
    
    mod2_init_mat[,i] = c(qlogis(c(aux_tpm_row1[1,2:3],
                                   aux_tpm_row2[1,c(1,3)],
                                   aux_tpm_row3[1,1:2])),#rnorm(6,-3,5), # tpm k = 1
                          qlogis(aux_inits[1,2:3]), # init k = 1
                          qlogis(aux_theta[2]), # pi
                          qlogis(aux_inits[2,2:3]), # init k = 2
                          qlogis(c(aux_tpm_row1[2,2:3],
                                   aux_tpm_row2[2,c(1,3)],
                                   aux_tpm_row3[2,1:2])),#rnorm(6,-3,5), # tpm k = 2
                          qlogis(aux_theta[3]), # pi for k = 3
                          qlogis(aux_inits[3,2:3]), # init k = 3
                          qlogis(c(aux_tpm_row1[3,2:3],
                                   aux_tpm_row2[3,c(1,3)],
                                   aux_tpm_row3[3,1:2])),#rnorm(6,-3,5), # tpm k = 3
                          qlogis(aux_theta[4]), # pi for k = 4
                          qlogis(aux_inits[4,2:3]), # init k = 4
                          qlogis(c(aux_tpm_row1[4,2:3],
                                   aux_tpm_row2[4,c(1,3)],
                                   aux_tpm_row3[4,1:2])))#rnorm(6,-3,5)) # tpm k = 3
  }
...
if(opt$kcontext == 5){
  ...
  for(i in 1:1500){
    ...
    aux_tpm_row1 = rdirichlet(5,alpha=c(10,1,1))
    aux_tpm_row2 = rdirichlet(5,alpha=c(1,10,1))
    aux_tpm_row3 = rdirichlet(5,alpha=c(1,1,10))
    ...
    mod2_init_mat[,i] = c(qlogis(c(aux_tpm_row1[1,2:3],
                          ...
                          qlogis(c(aux_tpm_row1[4,2:3],
                                   aux_tpm_row2[4,c(1,3)],
                                   aux_tpm_row3[4,1:2])),#rnorm(6,-3,5)) # tpm k = 4
                          qlogis(aux_theta[5]), # pi for k = 5
                          qlogis(aux_inits[5,2:3]), # init k = 5
                          qlogis(c(aux_tpm_row1[5,2:3],
                                   aux_tpm_row2[5,c(1,3)],
                                   aux_tpm_row3[5,1:2])))#rnorm(6,-3,5)) # tpm k = 5
