functions {
  ...
  //vector pi_
    ...
    for(k in 1:K){
      ...
      matrix[N,N] aux_tpm = tpm[k];
      vector[N] aux_init = init[k];
      for(i in 1:n_ind){
        for(state in 1:N){
        ...
        for(i in 1:n_ind){
          // log_alpha_components[k,i] += log(pi_[k]) + log_sum_exp(log_alpha[,i]);
          log_alpha_components[k,i] = log_sum_exp(log_alpha[,i]);
          }
      }
      for(i in 1:n_ind){
        // log_lik_total += log_sum_exp(log_alpha_components[,i]);
        log_lik_total += log_alpha_components[1,i];
      }
...
data{
    int K; // Total number of contexts/scenarios
...
parameters{
 ...
 array[K] vector[N-1] init_raw;

 // rows tpm
 array[K] vector[N-1] tpm1;
 array[K] vector[N-1] tpm2;
 array[K] vector[N-1] tpm3;
...
transformed parameters{
  ...
  // init
  array[K] vector[N] init;
  // tpm
  array[K] matrix[N,N] tpm;
  
  // weights
  // vector[K] pi_;
  // for(k in 2:K){
  //   pi_[k] = 1/(1+exp(-pi_raw[k-1]));
  // }
  // pi_[1] = 1-sum(pi_[2:K]);
  // pi_[2] = 1/(1+exp(-pi_raw[1]));
  // pi_[3] = 1/(1+exp(-pi_raw[2]));
  // pi_[1] = 1- sum(pi_[2:3]);
 
  
  for(k in 1:K){
    init[k,2] = 1/(1+exp(-init_raw[k,1]));
    init[k,3] = 1/(1+exp(-init_raw[k,2]));
    init[k,1] = 1- sum(init[k,2:3]);
  }
  ...
  //Creation tpm
  for(k in 1:K){
    tpm[k,1,1] = 1 - sum(exp(tpm1[k,]))/(1 + sum(exp(tpm1[k,])));
    tpm[k,1,2] = exp(tpm1[k,1])/(1 + sum(exp(tpm1[k,])));
    tpm[k,1,3] = exp(tpm1[k,2])/(1 + sum(exp(tpm1[k,])));
    
    tpm[k,2,1] = exp(tpm2[k,1])/(1 + sum(exp(tpm2[k,])));
    tpm[k,2,2] = 1 - sum(exp(tpm2[k,]))/(1 + sum(exp(tpm2[k,])));
    tpm[k,2,3] = exp(tpm2[k,2])/(1 + sum(exp(tpm2[k,])));
    
    tpm[k,3,1] =   exp(tpm3[k,1])/(1 + sum(exp(tpm3[k,])));
    tpm[k,3,2] = exp(tpm3[k,2])/(1 + sum(exp(tpm3[k,])));
    tpm[k,3,3] = 1 - sum(exp(tpm3[k,]))/(1 + sum(exp(tpm3[k,])));
