log.likelihood_v2_mod2 = function(list_data,
                             theta_star){
...
theta_star_2 = c(rep(0,6), # tpm k = 1
...
stan_data$K = 2
-log.likelihood_v2(stan_data,theta_star)
-log.likelihood_v2_mod2(stan_data,theta_star_2)

t1 = Sys.time()
result = nlm(f=log.likelihood_v2_mod2,p=theta_star_2,list_data = stan_data)
t2 = Sys.time()
t2-t1
saveRDS(result,"mod2/nlm_output_mod2.rds")



# TODO
# Step 1: Fix values for parameters for the state-dependent distributions (mu, sigma and the others) -- done
# Step 2: Simulate values for initial distribution and tpm rows (use multinomial or bernoulli) -- done
# Step 3: From 15,000 iterarions, select the best 100 sets with the highest log-lik

# Step 4: (to consult with Vianey) add noise to these 100 sets, 5 times, and rerun optimization
