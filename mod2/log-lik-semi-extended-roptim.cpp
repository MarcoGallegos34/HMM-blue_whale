...
struct MleEstim : public Worker
{
    ...
    // void semiMleExtended_bfgs(    
    double semiMleExtended_bfgs_parallel(    
        arma::Col<int> ID_init,
        const arma::Col<int> ID,
        const arma::Col<double> x_duration_init,
        const arma::Col<double> x_surface_init,
    ...
    void operator() (std::size_t begin, std::size_t end)
    {
        ...
        for (std::size_t i = begin; i < end; i++)
        {
            ...
            output[i] = semiMleExtended_bfgs_parallel(aux_ID_init,
                        aux_ID,
                        aux_x_duration_init,
                        aux_x_surface_init,
                        aux_x_maxDepth_init,
...
// void MleEstim::semiMleExtended_bfgs(    
double MleEstim::semiMleExtended_bfgs_parallel(    
    ...
    arma::Col<double> theta_star){
        // SemiMleExtended sm = new SemiMleExtended(
        SemiMleExtended sm(
            K,
            N,
            n,
        ...
        // delete sm;
        return opt.value();
            // double result = semiMleExtended_bfgs(
            // K,
            // N,
            // n,
            // n_ind,
...
// [[Rcpp::export]]
NumericVector parallel_mle_estimator_extended(NumericMatrix Y,
                                const int K,
                                const int N,
                                const int n,
                                const int n_ind,
    ...
    NumericVector output_parallel(Y.ncol());
