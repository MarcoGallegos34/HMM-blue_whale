...
for(i in 1:500){
  ...
  mod3_init_mat[,i] = c(c(rnorm(1,-5+.997,2),rnorm(1,-5+27.1,2),
                          rnorm(1,-1.25 -17,2),rnorm(1,-5-.147,2),
                          rnorm(1,-4.5 + 1.24,2),rnorm(1,-5 - .390,2)), # tpm k = 1
                        #qlogis(aux_inits[1,2:3]), # init k = 1
                        c(-6,-1.65) + rnorm(2,0,2), # init k = 1
                        .2642 + rnorm(1,0,2), # pi
                        # qlogis(.449) + rnorm(1,0,1), # pi
                        c(-.525,-5) + rnorm(2,0,2), # init k = 2
                        # qlogis(c(.37,.001)) + rnorm(2,0,1), # init k = 2
                        c(rnorm(1,-8+.997,2),rnorm(1,-31.5+27.1,2),
                          rnorm(1,-5-17,2),rnorm(1,-1.35-.147,2),
                          rnorm(1,-4+1.24,2),rnorm(1,-1.2-.39,2)), # tpm k = 2
                        -1.9 + rnorm(1,0,2), # pi for k = 3
                        # qlogis(.054) + rnorm(1,0,1), # pi for k = 3
                        c(-0,-5) + rnorm(2,0,2), # init k = 3
                        # qlogis(c(.5,.001)) + rnorm(2,0,1), # init k = 3
                        c(rnorm(1,-2.9+.997,2),rnorm(1,-27.52 + 27.1,2),
                          rnorm(1,-5-17,2),rnorm(1,-1.95-.147,2),
                          # rnorm(1,-3.88+1.24,1),rnorm(1,-2-.39,1)), # tpm k = 3
                          rnorm(1,1.23+1.24,2), rnorm(1,0.26-.39,2)), # tpm k = 3
                        -.8 + rnorm(1,0,2), # pi for k = 4
                        # qlogis(.163) + rnorm(1,0,1), # pi for k = 4
                        c(-5,-.67) + rnorm(2,0,2), # init k = 4
                        # qlogis(c(.001,.34)) + rnorm(1,0,1), # init k = 4
                        c(rnorm(1,-1.55+.997,2),rnorm(1,-26.55+27.1,2),
                          rnorm(1,-4-17,2),rnorm(1,.42-.147,2),
                          # rnorm(1,-1.74+1.24,1),rnorm(1,1.37-.39,1)), # tpm k = 4
                          rnorm(1,-2.18+1.24,2),rnorm(1,-1.1-.39,2)), # tpm k = 4
                        c(-.997,-27.1,17,.147,-1.24,.39) + rnorm(6,0,2)) # covariates beta
}
