...
mod4_init_mat = matrix(NA,41+18,500)
...
for(i in 1:500){
  ...
  mod4_init_mat[,i] = c(rnorm(6,0,5), # tpm k = 1
                        rnorm(2,0,5),#qlogis(aux_inits[1,2:3]), # init k = 1
                        rnorm(1,0,5),#qlogis(aux_theta[2]), # pi
                        rnorm(2,0,5),#qlogis(aux_inits[2,2:3]), # init k = 2
                        # c(rnorm(1,-4.5,3),rnorm(1,-5,3),rnorm(1,-27,5),rnorm(1,-4,3),rnorm(1,-2.8,2),rnorm(1,-1.6,2)), # tpm k = 2
                        rnorm(6,0,5), # tpm k = 2
                        rnorm(1,0,5),#qlogis(aux_theta[3]), # pi for k = 3
                        rnorm(2,0,5),#qlogis(aux_inits[3,2:3]), # init k = 3
                        # c(rnorm(1,-1.9,2),rnorm(1,-.45,2),rnorm(1,-10,2),rnorm(1,-2.1,1.5),rnorm(1,2,1.5),rnorm(1,-.6,1.5)), # tpm k = 3
                        rnorm(6,0,5), # tpm k = 3
                        rnorm(1,0,5),#qlogis(aux_theta[4]), # pi for k = 4
                        rnorm(2,0,5),#qlogis(aux_inits[4,2:3]), # init k = 4
                        # c(rnorm(1,-.3,1.5),rnorm(1,1,1.5),rnorm(1,-10,2),rnorm(1,.2,1.5),rnorm(1,-1,1.5),rnorm(1,-1.5,1.5)), # tpm k = 4
                        rnorm(6,0,5), # tpm k = 4
                        rnorm(24,0,5)) # covariates beta
}

sourceCpp("mod4/log-lik-semi-mod4-roptim.cpp")
...
saveRDS(mle_vec,paste0("mod4/semiMles_mod4-v2-",opt$seed,".RDS"))
