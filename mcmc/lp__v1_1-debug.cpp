...
NumericVector next_point(const NumericVector x_old){
    ...
    NumericVector aux_x_new(x_old.size());
    for(int i=0; i < x_old.size();i++){
        aux_x_new[i] = x_new[i]/aux_sum;
    }
    return aux_x_new;
}
...
struct ComponentWiseStep : public Worker
{
    ...
    RMatrix<double> output; // output vector 
        ...
        for (std::size_t i = begin; i < end; i++)
        {
            ...
            if( i == 2){
                ...
                if(log_U[2] <= log_ratio){
                    output(1-1,0) = the_Y_n_matrix(1-1,1-1);
                    output(2-1,0) = the_Y_n_matrix(2-1,1-1);
                    output(44,0) = the_Y_n_matrix(44,1-1);
                } else {
                    output(1-1,0) = X_i[1-1];
                    output(2-1,0) = X_i[2-1];
                    output(44,0) = X_i[44];
                }
                output(1-1,1) = the_target_at_X_i;
                output(2-1,1) = the_target_at_X_i;
                output(44,1) = the_target_at_X_i;

                output(1-1,2) = the_target_at_proposal;
                output(2-1,2) = the_target_at_proposal;
                output(44,2) = the_target_at_proposal;
            ...
            }  else if ( i == 3)
            {
                ...
                if(log_U[3] <= log_ratio){
                    output(3-1,0) = the_Y_n_matrix(3-1,3-1);
                    output(4-1,0) = the_Y_n_matrix(4-1,3-1);
                    output(45,0) = the_Y_n_matrix(45,3-1);
                } else {
                    output(3-1,0) = X_i[3-1];
                    output(4-1,0) = X_i[4-1];
                    output(45,0) = X_i[45];
                }
                
                output(3-1,1) = the_target_at_X_i;
                output(4-1,1) = the_target_at_X_i;
                output(45,1) = the_target_at_X_i;

                output(3-1,2) = the_target_at_proposal;
                output(4-1,2) = the_target_at_proposal;
                output(45,2) = the_target_at_proposal;
            ...
            } else if ( i == 4)
            {
                ...
                if(log_U[4] <= log_ratio){
                    output(5-1,0) = the_Y_n_matrix(5-1,5-1);
                    output(6-1,0) = the_Y_n_matrix(6-1,5-1);
                    output(46,0) = the_Y_n_matrix(46,5-1);
                } else {
                    output(5-1,0) = X_i[5-1];
                    output(6-1,0) = X_i[6-1];
                    output(46,0) = X_i[46];
                }
                output(5-1,1) = the_target_at_X_i;
                output(6-1,1) = the_target_at_X_i;
                output(46,1) = the_target_at_X_i;

                output(5-1,2) = the_target_at_proposal;
                output(6-1,2) = the_target_at_proposal;
                output(46,2) = the_target_at_proposal;
            ...
            } else if ( i == 5)
            {
                ...
                if(log_U[5] <= log_ratio){
                    output(42,0) = the_Y_n_matrix(42,42);
                    output(43,0) = the_Y_n_matrix(43,42);
                    output(47,0) = the_Y_n_matrix(47,42);
                } else {
                    output(42,0) = X_i[42];
                    output(43,0) = X_i[43];
                    output(47,0) = X_i[47];
                }
                output(42,1) = the_target_at_X_i;
                output(43,1) = the_target_at_X_i;
                output(47,1) = the_target_at_X_i;

                output(42,2) = the_target_at_proposal;
                output(43,2) = the_target_at_proposal;
                output(47,2) = the_target_at_proposal;
                ...
                // if(i != 48 && i != 49 && i !=50){
                if(i < (size - 3)){
                    log_alpha +=  std::log(the_Y_n_matrix(i,i)) - std::log(X_i[i]);
                }
                ...
                if(log_U[i] <= log_ratio){
                    output(i,0) = the_Y_n_matrix(i,i);
                } else {
                    output(i,0) = X_i[i];    
                }
                output(i,1) = the_target_at_X_i;
                output(i,2) = the_target_at_proposal;
            }
...
List rcpp_parallel_pt_cw_M_target_posterior(const List list_data, 
    ...
    // List where each element will allocate samples from the different tempered versions of the target
    List X(temp_vector.size());
    NumericMatrix mat_target_X_i(init.size(),nsim+1);
    NumericMatrix mat_target_proposal(init.size(),nsim+1);
    NumericMatrix mat_proposal(init.size(),nsim+1);
    // Initial values provided for each tempered target
    for(int i = 0; i < temp_vector.size(); ++i){
        X[i] = createMatrix(init.size(),nsim+1);
        NumericMatrix aux_X = X[i];
    ...
    for(int i=0; i<nsim; ++i){
        ...
        for(int temp = 0; temp < temp_vector.size(); ++temp){
            ...
            // new_init = next_point(dummy_vec);
            // new_tpm1 = next_point(dummy_vec);
            // new_tpm2 = next_point(dummy_vec);
            // new_tpm3 = next_point(dummy_vec);
            ...
            NumericMatrix output_parallel(init.size(),3);
            ...
            // for(int j = 0; j < 6; j++){
            //     aux_X(j,i+1) = output_parallel(j,0);
            //     if(temp == 0){
            //         mat_target_X_i(j,i+1) = output_parallel(j,1);
            //         mat_target_proposal(j,i+1) = output_parallel(j,2);
            //         mat_proposal(j,i+1) = aux_Y_n[j];
            //     }
            // }
    ...
    return List::create(_["chain1"] = X[0] , _["target_X_i"] = mat_target_X_i, _["target_proposal"] = mat_target_proposal, _["proposal"] = mat_proposal);
