...
struct ComponentWiseStep : public Worker
{
    ...
    const RMatrix<double> X_i_matrix; // samples from iteration i
    // const RVector<double> the_target_at_X_i; // value of the tempered target at the iteration i
    ...
    arma::mat convert_X_i(){
        RMatrix<double> tmp_X_i_matrix = X_i_matrix;
        arma::mat MAT_X(tmp_X_i_matrix.begin(), size, size, false);
        return MAT_X;
    }
    ...
    ComponentWiseStep (
            ...
            // other parameters
            const NumericMatrix X_i_matrix_in,
    ...
    void operator() (std::size_t begin, std::size_t end)
    {
        ...
        for (std::size_t i = begin; i < end; i++)
        {
            ...
            arma::mat MAT_X_i = convert_X_i();

            if( i == 2){

                arma::Col<double> col_i = MAT.col(0);
                arma::Col<double> col_i_X_i = MAT_X_i.col(0);
                ...
                double the_target_at_X_i = target_tau_(aux_ID_init,
                            aux_ID,
                            aux_x_duration_init,
                            aux_x_surface_init,
                            aux_x_maxDepth_init,
                ...
                if(log_U[2] <= log_ratio){
                    ...
                    output[1-1] = X_i_matrix(1-1,1-1);
                    output[2-1] = X_i_matrix(2-1,1-1);
                    output[44] = X_i_matrix(44,1-1);
                }

            }  else if ( i == 3)
            {
                arma::Col<double> col_i = MAT.col(2);
                arma::Col<double> col_i_X_i = MAT_X_i.col(2);
                double log_alpha = 0.0;
                double log_ratio = 0.0;
                double the_target_at_proposal = target_tau_(aux_ID_init,
                            aux_ID,
                ...
                if(log_U[3] <= log_ratio){
                    ...
                    output[3-1] = X_i_matrix(3-1,3-1);
                    output[4-1] = X_i_matrix(4-1,3-1);
                    output[45] = X_i_matrix(45,3-1);
                }

            } else if ( i == 4)
            {
                arma::Col<double> col_i = MAT.col(4);
                arma::Col<double> col_i_X_i = MAT_X_i.col(4);
                double log_alpha = 0.0;
                double log_ratio = 0.0;
                double the_target_at_proposal = target_tau_(aux_ID_init,
                            aux_ID,
                ...
                if(log_U[4] <= log_ratio){
                    ...
                    output[5-1] = X_i_matrix(5-1,5-1);
                    output[6-1] = X_i_matrix(6-1,5-1);
                    output[46] = X_i_matrix(46,5-1);
                }

            } else if ( i == 5)
            {
                arma::Col<double> col_i = MAT.col(42);
                arma::Col<double> col_i_X_i = MAT_X_i.col(42);
                double log_alpha = 0.0;
                double log_ratio = 0.0;
                double the_target_at_proposal = target_tau_(aux_ID_init,
                            aux_ID,
                ...
                if(log_U[5] <= log_ratio){
                    ...
                    output[42] = X_i_matrix(42,42);
                    output[43] = X_i_matrix(43,42);
                    output[47] = X_i_matrix(47,42);
                }

            } else{
                arma::Col<double> col_i = MAT.col(i);
                arma::Col<double> col_i_X_i = MAT_X_i.col(i);
                ...
                if(i < (size - 3)){
                    ...
                    log_alpha -=  std::log(X_i_matrix(i,i));
                }
...
void vector_aggregator(NumericMatrix Y,
                                ...
                                NumericMatrix X_i, NumericVector vec_log_U, double temp, double maxtemp, int begin_in, int end_in){
    ...
    ComponentWiseStep componentWiseStep(Y,
                                        ...
                                        x_headVar,
                                        //other parameters    
                                        X_i,size, temp, maxtemp,vec_log_U,
                                        output_parallel);
...
List rcpp_parallel_pt_cw_M_target_posterior(const List list_data, 
    ...
    for(int i=0; i<nsim; ++i){
        ...
        for(int temp = 0; temp < temp_vector.size(); ++temp){
            ...
            matrix_aux_X(2-1,1-1) =  aux_X(2-1,i);
            matrix_aux_X(44,1-1) =  aux_X(44,i);

            // completition tpm2 in matrix_Y_n
            matrix_Y_n(4-1,3-1) =  aux_Y_n[4-1];
            matrix_Y_n(45,3-1) =  aux_Y_n[45];

            matrix_aux_X(4-1,3-1) =  aux_X(4-1,i);
            matrix_aux_X(45,3-1) =  aux_X(45,i);

            // completition tpm2 in matrix_Y_n
            matrix_Y_n(6-1,5-1) =  aux_Y_n[6-1];
            matrix_Y_n(46,5-1) =  aux_Y_n[46];

            matrix_aux_X(6-1,5-1) =  aux_X(6-1,i);
            matrix_aux_X(46,5-1) =  aux_X(46,i);
            ...
            matrix_aux_X(43,42) = aux_X(43,i);
            matrix_aux_X(47,42) = aux_X(47,i);
            ...
            // tempered target at current iteration
            // NumericVector target_at_X_i(init.length());
            // for(int j = 0; j < init.size(); j++){
            //     target_at_X_i[j] = target_tau(list_data, matrix_aux_X(_,j),temp_vector[temp],temp_vector[temp_vector.size()-1]);
            ...
            vector_aggregator(matrix_Y_n,
                                                                ...
                                                                x_headVar,
                                                                output_parallel,
                                                                matrix_aux_X,vec_log_U,temp_vector[temp],temp_vector[temp_vector.size()-1],2,init.length()-9);
            ...
            vector_aggregator(matrix_Y_n,
                                                    ...
                                                    // aux_parallel_Y_n,
                                                    output_parallel,
                                                    matrix_aux_X,vec_log_U,temp_vector[temp],temp_vector[temp_vector.size()-1],init.length()-3,init.length());
